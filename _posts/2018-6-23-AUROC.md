...
So, if for each threshold value $$T$$, we plot the **TFR** on x-axis, and **TPR** on y-axis , we get the **Receiver Operating Characteristic (ROC)** curve. Hence, we can merge the two metrics **TFR** and **TPR** into one metric **AUROC**, that is independent to the threshold by computing the Area Under this Curve (**AUC**).
### How to compute it?
Let say you apply the prediction to all test samples, and rank the samples by their score $$p$$, as illustrated in
